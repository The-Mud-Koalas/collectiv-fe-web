...
const SERVICE_INITIATIVES = {
  ...
  description: "Promote an inclusive and respectful atmosphere and encourage attendees to be mindful of different beliefs.",
}
...
const EventType: React.FC<Props> = ({ currentStage }) => {
  ...
  return (
    <EventCollapsible
      isCollapsibleEnabled={currentStage === 0}
      sectionId="event-type"
      sectionTitle="Event Type"
      description={EVENT_DESCRIPTION}
      childrenIfCollapsed={<EventTypeCard {...chosenType} />}
      initiallyOpened
    >
      <div className="flex flex-col gap-3 w-full">
        ...
        <EventTypeChoice {...SERVICE_INITIATIVES} clickHandler={changeIsProject(false)} selectedWhen={!isProject}/>
        <EventTypeChoice {...COMMUNITY_PROJECTS} clickHandler={changeIsProject(true)} selectedWhen={isProject}/>
      </div>
    </EventCollapsible>
  );
};
